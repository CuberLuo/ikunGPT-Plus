...
async function onConversation() {
  ...
  try {
    ...
    for (let i = 0; i < errorStr.length; i++) {
      if (errorStr[i] === '4') {
        code = errorStr.substring(i, i + 3)
        break
      }
      else if (errorStr[i] === '5') {
        code = errorStr.substring(i, i + 3)
        break
      }
    }
    ...
    scrollToBottomIfAtBottom()
    if (code === '412')
      ms.error(t('chat.chatError1'))
    else if (code === '500')
      ms.error(t('chat.chatError2'))
    else
      ms.error(t('chat.chatError3'))
  }
  finally {
    loading.value = false
  }
